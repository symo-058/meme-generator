...
function showJoke() {
  ...
  const newP = document.querySelector('.joke-content');
  const newJ = document.createElement('p');
  newJ.textContent =randomJokeText;
  
  
  clearAll();
  newP.appendChild(newJ);
...
function showQuote() {
  ...
  const contain = document.querySelector('.quote-content')
...
function showRiddle() {
  ...
  clearAll();
  ridd.appendChild(riddle);
  ridd.appendChild(answer);
}
